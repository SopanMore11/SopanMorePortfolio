    ...
    <section id="skills" className="py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        <div className="text-center mb-20 animate-slide-up">
          <h2 className="text-4xl sm:text-5xl font-bold mb-6 gradient-text" data-testid="skills-title">
            Technical Expertise
          </h2>
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Cutting-edge technologies powering the next generation of AI solutions
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {Object.entries(skills).map(([category, skillList], categoryIndex) => {
            ...
            const levels = skillLevels[category as keyof typeof skillLevels] || [];
            ...
            return (
              <Card 
                ...
                data-testid={`skills-category-${category.toLowerCase().replace(/[^a-z0-9]/g, '-')}`}
              >
                <CardContent className="p-8">
                  <div className="flex items-center mb-8">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold mb-1" data-testid={`skills-category-title-${category.toLowerCase().replace(/[^a-z0-9]/g, '-')}`}>
                        {category}
                      </h3>
                      ...
                      return (
                        <div key={index} className="group">
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-medium text-sm group-hover:text-primary transition-colors" data-testid={`skill-${category.toLowerCase().replace(/[^a-z0-9]/g, '-')}-${index}`}>
                              {skill}
                            </span>
                            <div className="flex items-center space-x-1">
                              {[...Array(5)].map((_, starIndex) => (
                                <Star 
                                  key={starIndex}
                                  className={`w-3 h-3 ${starIndex < Math.round(level / 20) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                                />
                              ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-slide-up">
          <div className="text-center p-6 rounded-2xl bg-gradient-to-br from-blue-500/10 to-blue-600/5 border border-blue-200/20">
            ...
            <p className="text-sm text-muted-foreground">Expert in LLMs, Computer Vision, and Deep Learning architectures</p>
          </div>
          ...
          <div className="text-center p-6 rounded-2xl bg-gradient-to-br from-purple-500/10 to-purple-600/5 border border-purple-200/20">
            ...
            <p className="text-sm text-muted-foreground">Proficient in Python, JavaScript, and modern web technologies</p>
          </div>
          ...
          <div className="text-center p-6 rounded-2xl bg-gradient-to-br from-cyan-500/10 to-cyan-600/5 border border-cyan-200/20">
            ...
            <p className="text-sm text-muted-foreground">Experienced with Azure Cloud services and scalable AI deployment</p>
          </div>
        </div>
      </div>
    </section>
